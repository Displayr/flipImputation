...
# m = 10
# est.data <- flipData::EstimationData(Overall ~ Fees + Interest + Phone + Branch + Online + ATM, bank, m = 10,
#                                        missing = "Multiple Imputation")
#                            seed = 123)
# datas <- est.data$estimation.data
#
# datmodels <- lapply(datas, FUN = function(x) lm(Overall ~ Fees + Interest + Phone + Branch + Online + ATM, data = x))
#
# dat <- datas[[1]]
# lm(Overall ~ Fees + Interest + Phone + Branch + Online + ATM, data = dat)
#
#
# library(flipRegression)
# models <- lapply(datas, FUN = function(x) Regression(Overall ~ Fees + Interest + Phone + Branch + Online + ATM, data = x))
#
#
